...
public:
        ...
        for (long i = 0; i < totalTests; i++) 
	{
            ...
            getline(infile, tmp);
			int N = stoi(tmp);
			testCase.N = pow(2, (int)ceil((float)log2(N)));
			cout<<"testCase.N:"<<testCase.N<<endl;
            getline(infile, tmp);
            testCase.K = stoi(tmp);
            getline(infile, tmp);
    ...
    template <typename T>
static void generateTwoChoiceTestCases(std::vector<TC<T> >& testCases, ulong keywordLength, unsigned long seed, bool overwrite, vector<string> testKeywords)	
    {
        char alphanum[] =
                "0123456789"
                "!@#$%^&*"
